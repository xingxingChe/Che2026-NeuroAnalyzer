# -*- coding: utf-8 -*-
"""
神经元放电互相关分析脚本 (V4 - 严格检验版)
功能:
1. 恢复使用论文 (Sauer & Bartos, 2022, eLife) 中定义的严格统计检验阈值。
2. 修复了总结文件(Summary.xlsx)的读写bug，确保文件格式正确。
3. 自动处理包含神经元放电时间戳的 .xlsx 文件。
4. 计算锥体细胞 (PYR) 到中间神经元 (INT) 的互相关图 (Cross-Correlation Histogram, CCH)。
5. 自动生成和导出分析图表、原始数据及总结报告。
"""
...
def calculate_spike_transmission_prob(cch, baseline, bins, pyr_spike_count):
    """计算尖峰传递概率。"""
    mono_window = (bins >= 0.0008) & (bins <= 0.0028)
    excess_spikes = np.sum(cch[mono_window]) - np.sum(baseline[mono_window])
    if pyr_spike_count == 0 or excess_spikes < 0:
        return 0.0
    return excess_spikes / pyr_spike_count
...
def plot_and_save_cch(cch, baseline, bins, pair_id, animal_id, is_connected, output_folder):
    ...
    ax.bar(bins, cch, width=bar_width, color='black', alpha=0.8, label='Raw CCH')
    ax.plot(bins, baseline, color='red', linewidth=2, linestyle='--', label='Baseline')
    ax.axvspan(0.0008, 0.0028, color='blue', alpha=0.2, label='Monosynaptic Window')
    ax.set_title(f"Cross-Correlation Histogram (CCH): {animal_id} - {pair_id}", fontsize=16)
    ...
    filename = f"{date_str}_CCH_{animal_id}_{pair_id}_{suffix}.png"
...
def main():
    ...
    # =========================================================================
    # *** 检验水平参数 ***
    # 已恢复为论文中使用的严格检验标准 (Sauer & Bartos, 2022, eLife)
    ...
    date_str = datetime.now().strftime('%Y%m%d')
    main_output_folder_name = f"{date_str}_E.2.3.4_Cross_Correlation_Histogram"
    main_output_path = os.path.join(output_folder, main_output_folder_name)
    os.makedirs(main_output_path, exist_ok=True)

    summary_file_name = f"{date_str}_Cross_Correlation_Summary.xlsx"
    ...
    try:
        with tqdm(total=len(files_to_process), desc="Overall Progress") as pbar_files:
            for filename in files_to_process:
                ...
                numeric_results_folder_name = f"{date_str}_CCH_Numeric_Results_{animal_id}"
                ...
                for i, n_type in enumerate(neuron_types):
                    ...
                    if len(spike_times) >= 500:
                        if n_type == 1:
                            ...
                            int_neurons[neuron_id] = spike_times

                total_pairs = len(int_neurons) * len(pyr_neurons)
                with tqdm(total=total_pairs, desc=f"{animal_id} Neuron Pairs", leave=False) as pbar_pairs:
                    for int_id, int_spikes in int_neurons.items():
                        for pyr_id, pyr_spikes in pyr_neurons.items():
                            pair_id = f"{int_id}-{pyr_id}"
                            pbar_pairs.set_description(f"Analyzing: {pair_id}")

                            cch, bins = calculate_cch(pyr_spikes, int_spikes)
                            ...
                            if is_connected:
                                transmission_prob = calculate_spike_transmission_prob(
                                    cch, baseline, bins, len(pyr_spikes)
                                )
                                # 将结果追加到内存列表
                                summary_data.append({
                                    "Animal_ID": animal_id,
                                    "Neuron_Pair_ID": pair_id,
        ...
        if summary_data:
            ...
            print("\nNo synaptic connections were detected across all files. An empty summary file was created.")
            # 创建一个空的带表头的xlsx文件
            pd.DataFrame(columns=["Animal_ID", "Neuron_Pair_ID", "Spike_Transmission_Probability"]).to_excel(
                summary_file_path, index=False)

        messagebox.showinfo("Complete", "All analyses have been completed successfully!")
